// ============================================================================
// printer.js  (API v1) — element de parc "Printer" (Imprimante)
// ----------------------------------------------------------------------------
// Champs GLPI reels : name, states_id, locations_id, manufacturers_id,
// printermodels_id, otherserial, contact.
// ============================================================================
...
export default class Printer {
  static itemtype = 'Printer'
  static endpoint = '/Printer'
  static label = 'Imprimante'
  static labelPluriel = 'Imprimantes'
  ...
  async toInput() {
    return {
      ...
      printermodels_id: await resolveDropdown('PrinterModel', this.model),
  ...
  async create() {
    const cree = await post(Printer.endpoint, await this.toInput())
    return cree.id
  }
  ...
  static getAll(options = {}) {
    ...
    return get(Printer.endpoint + query)
  }

  static getOne(id) {
    return get(Printer.endpoint + '/' + id)
  }

  static remove(id) {
    return del(Printer.endpoint + '/' + id + '?force_purge=true')
  }

  static toDisplay(brut) {
    return {
      id: brut.id,
      type: Printer.itemtype,
      ...
      model: nettoyer(brut.printermodels_id),
